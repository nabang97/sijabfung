...
var tableGolongan = $("#tableGolongan").DataTable({
    ...
    columns: [
        ...
        {
            defaultContent:
                '<button type="button" id="btnEditGolongan" data-toggle="modal" data-target="#modalEditGolongan" class="btn btn-sm btn-primary" >Edit</button> <button type="button"  id="btnRemoveGolongan" class="btn btn-sm btn-primary">Remove</button>'
        }
    ]
});
...
$("#tableGolongan tbody ").on("click", "button", function() {
    ...
    if (this.id == "btnEditGolongan") {
        $("#modalEditGolongan").on("show.bs.modal", function(e) {
            $(this)
                .find('input[name="idGolongan"]')
                .val(data.id);
            $(this)
                .find('input[name="namaGolongan"]')
                .val(data.name);
        });
    }
...
const deleteGolongan = data => {
    $.post("/api/golongan/destroy", { id: data, _token: CSRF_TOKEN })
        .done(function(e) {
...
const updateGolongan = (id, name) => {
    $.post("/api/golongan/update", { id: id, name: name, _token: CSRF_TOKEN })
        .done(function(e) {
            tableGolongan.ajax.reload();
        })
        .fail(function(e) {})
